    ...
    <div class="product-container">
        <div class="product-menu-container bg-fff box-shadow padding-2-pre u-m-t-10">
            ...
            <div class="product-menu">
                <div class="product u-cursor" v-for="(product, index) in productMenuList" :key="index">
                    <div class="left">
                        <div class="title u-font-22 u-font-weight">{{ product.title }}</div>
                        <div class="viceTitle u-font-16">{{ product.viceTitle }}</div>
                        <div class="version-container">
                            <div class="version" v-for="(version) in product.version" :key="version.id">
                                {{ version.name }}
                            </div>
                        </div>
                ...
                <div class="config-items" v-for="(product, index) in _productConfigList" :key="index">
                    ...
                    <div class="config-item">
                        <div class="item" v-for="(option, _index) in product[1]" :key="_index">
                            ...
                            <div class="right">
                                <el-checkbox @change="handleChangeCheckbox(option.optionId)" :checked="option.isEssential" :disabled="option.isEssential" size="large"/>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="settle-accounts-container bg-fff box-shadow padding-2-pre u-m-t-10">
            <div class="left">
                <div class="product-option">
                    <div class="text">吗哩呀咔-智能助手</div>
                    <div class="version">专业版</div>
                    <div class="count">x 1</div>
                </div>
                <div class="product-option">选配产品 x 2</div>
            </div>
            <div class="right">
                <div class="total-money">共计<span>99999.99</span>元</div>
...
<script setup>
    ...
    const checkedOptions = reactive(new Set())

    function handleFormatConfigList(ConfigList) {
      let configMap = new Map();

      ConfigList.forEach(product=>{
        if (product.isEssential) checkedOptions.add(product.optionId)
        let list = configMap.has(product.type) ? configMap.get(product.type) : [];
        list.push(product)
        configMap.set(product.type, list)
      })

      _productConfigList.value = configMap
    }

    function handleChangeCheckbox(optionId) {
      if (checkedOptions.has(optionId)) {
        checkedOptions.delete(optionId)
      }else {
        checkedOptions.add(optionId)
      }
    }
