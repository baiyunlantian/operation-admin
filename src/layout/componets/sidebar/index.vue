                ...
                >
                    ...
                    <template v-if="menuItems === 'left'">
                        <template v-for="(menu, index) in leftMenuList" :key="menu.menuId">
                            <el-menu-item v-if="menu.children.length === 0 && (menu.permission.length === 0 || menu.permission.includes(userInfo.roleId))" class="menu-item"  :index="menu.path">
                                {{menu.name}}
                            </el-menu-item>

                            <el-sub-menu v-else-if="menu.children.length > 0 && (menu.permission.length === 0 || menu.permission.includes(userInfo.roleId))" class="sub-menu-item" :index="index+'b'">
                                <template #title>{{menu.name}}</template>
                                <template v-for="(secondMenu, sIndex) in menu.children" :key="secondMenu.menuId">
                                    <el-menu-item v-if="secondMenu.permission.length === 0 || secondMenu.permission.includes(userInfo.roleId)" :index="secondMenu.path">{{secondMenu.name}}</el-menu-item>
                                </template>
                            </el-sub-menu>
                        </template>
                    </template>
