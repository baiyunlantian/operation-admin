...
const utils = {
  ...
  cloneArray(val) {
    ...
    for (var i in val) {
      newobj[i] =
        typeof val[i] === "object" && val[i] !== null
          ? that.cloneArray(val[i])
          : val[i];
    }
    return newobj;
  },
  ...
  // 获取目标日期前多少天的具体日期
  getBeforeDate(beforeDays) {
    const beforeDaysStamp = beforeDays * 1000 * 60 * 60 * 24;
    const currentDateStamp = Date.now();
    const beforeDateStamp = currentDateStamp - beforeDaysStamp;
    const beforeDate = new Date(beforeDateStamp).toLocaleDateString();
    const beforeDateSplit = beforeDate.split("/");
